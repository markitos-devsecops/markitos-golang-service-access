...
func createRequestOrExitWithError(ctx *gin.Context) (services.UserUpdateRequest, bool) {
	...
	var request services.UserUpdateRequest = *services.NewUserUpdateRequest(requestUri.Id)
...
func (s *Server) userMotdHandler(ctx *gin.Context) {
	...
	ctx.JSON(http.StatusOK, "[USER] - Marco Antonio - markitos say, Hi all!! at "+time.Now().Format(time.RFC3339)+" on "+hostname)
}
